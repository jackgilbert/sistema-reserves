...
export class SettingsValidatorService {
   ...
   */
  validateFeatureFlags(flags: Partial<FeatureFlags>): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      flags.payments?.requireDeposit &&
      (flags.payments.depositPercentage ?? 0) <= 0
    ) {
    ...
    if (flags.bookings?.requirePaymentOnBooking && !flags.payments?.enabled) {
      errors.push(
        'requirePaymentOnBooking habilitado pero payments está deshabilitado.',
      );
    }
    ...
    if (
      ...
      flags.bookings.maxAdvanceBookingDays < 1
    ) {
      errors.push('maxAdvanceBookingDays debe ser al menos 1.');
    }

    if (
      flags.bookings?.minAdvanceBookingHours &&
      flags.bookings.minAdvanceBookingHours < 0
    ) {
    ...
    if (flags.payments?.depositPercentage) {
      if (
        flags.payments.depositPercentage < 0 ||
        flags.payments.depositPercentage > 100
      ) {
    ...
    if (
      flags.multiLanguage?.enabled &&
      (!flags.multiLanguage.supportedLocales ||
        flags.multiLanguage.supportedLocales.length === 0)
    ) {
   ...
   * Validar settings
   */
  validateSettings(settings: Partial<TenantSettings>): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      ...
      !this.isValidEmail(settings.general.contactEmail)
    ) {
      errors.push('contactEmail no tiene formato válido.');
    }

    if (
      settings.notifications?.fromEmail &&
      !this.isValidEmail(settings.notifications.fromEmail)
    ) {
    ...
    if (
      ...
      !this.isValidHexColor(settings.branding.primaryColor)
    ) {
    ...
    if (
      ...
      !this.isValidHexColor(settings.branding.secondaryColor)
    ) {
    ...
    if (
      ...
      !this.isValidHexColor(settings.branding.accentColor)
    ) {
    ...
    // Validar URLs
    if (
      settings.integrations?.customWebhookUrl &&
      !this.isValidUrl(settings.integrations.customWebhookUrl)
    ) {
    ...
    if (
      settings.booking?.defaultSlotDuration &&
      settings.booking.defaultSlotDuration < 5
    ) {
      errors.push('defaultSlotDuration debe ser al menos 5 minutos.');
    }

    if (
      settings.notifications?.reminderHoursBefore &&
      settings.notifications.reminderHoursBefore < 1
    ) {
      errors.push('reminderHoursBefore debe ser al menos 1 hora.');
    }

    if (
      settings.policies?.minBookingNoticeHours &&
      settings.policies.minBookingNoticeHours < 0
    ) {
    ...
    if (
      ...
      settings.policies.maxBookingAdvanceDays < 1
    ) {
    ...
    if (
      ...
      !['12h', '24h'].includes(settings.regional.timeFormat)
    ) {
  ...
  validateConsistency(
    ...
    if (
      ...
      settings.notifications?.sendBookingConfirmation
    ) {
      warnings.push(
        'Notifications feature deshabilitado pero sendBookingConfirmation está activo.',
      );
    }
    ...
    if (!flags.payments?.enabled && settings.integrations?.stripeEnabled) {
      warnings.push(
        'Payments feature deshabilitado pero Stripe está habilitado.',
      );
    }
    ...
    if (
      ...
      warnings.push(
        'maxAdvanceBookingDays en feature flags difiere de policies. Considere sincronizarlos.',
      );
    }
